...
~30 findings remain. None are attack chains of the same severity as C1-C9, no actionable Highs are left, the shared-library extraction (Group B) plus both reconciliation tails (Group I for Lows/Nits, Group J for Mediums) are closed, and the two Medium pattern-closes that required `_lib/` centralization (M7 and M4) are now shipped. What's left is 15 open Lows (all documented with current-state notes), 1 open Nit (N5 CORS preview-domain workflow concern), 13 open Mediums (all documented with current-state notes; includes the newly-filed M40), and H29 waiting on design.
...
**The audit is at a natural stopping point.** All Criticals closed, all non-deferred Highs closed, the Lows + Nits tail walked end-to-end (Group I), the Mediums tail walked end-to-end (Group J), L6 pre-task shipped, and the two shared-library Mediums (M7 supabase throws + M4 github allowlist) both closed via `_lib/` centralization. What remains falls into five categories, none of which require urgency:

1. **M40 wrapper-bypass migration** (≤1 session). Three routes issue raw `fetch` / `fetchT` GitHub calls that skip the `validatePath` allowlist landed in M4: `api/process-entity-audit.js` (9 raw ops across template reads + `<slug>/entity-audit/`, `<slug>/entity-audit-checkout/`, `<slug>/audits/<suite>/` deploys; imports `gh` at L13 but never uses it), `api/generate-proposal.js` (4 deploys under `<slug>/proposal|index|checkout|onboarding/`), and `api/run-migration.js` (read-only `migrations/<filename>` fetch, CRON_SECRET-gated, strong regex). Fix shape for (1)+(2): migrate to `gh.readTemplate` + `gh.pushFile`, which collapses each deploy from ~15 lines of hand-rolled scaffolding to 2. For (3): either add `migrations/` to the allowlist or leave standalone. Severity is Medium (no user-input reachability), not urgent. Filed as M40 during the M7+M4 session.
2. **H29 design session** (whenever ready). Waits on 4 design decisions captured in Group G batch 2 retrospective (JSONB encryption + read-path + migration + rotation). No code session will move it forward until those land.
3. **M3 Session 6 tightening** (product-gated, 0.5 session once decision lands). Flip `signed_agreements` + `payments` to `{ write: false, delete: false }` and `workspace_credentials` to `{ require_role: 'owner' }` in `api/_lib/action-schema.js`. The 3-line edit is trivial; the product call on whether `workspace_credentials` tightening breaks Scott's onboarding workflow is the gate.
4. **Group H M1 Stripe metadata** (10 min code + 30-day observation window). Blocked on dashboard-side `metadata: { product: ... }` addition.
5. **L29 top-line-string polish** (≤1 session, low value). Walk the ~40 callsites that still echo `'Supabase query error'` / `'Supabase mutate error'` in 5xx response bodies and replace each with a domain-appropriate string (`'Failed to load contact'`, `'Email send failed'`, etc.). Zero security value beyond what M7 already shipped — purely consistency / UX polish. Bundle with L25's cosmetic pass when one of you has a Friday afternoon free.

Or stop here with **"every Critical closed, every non-deferred High closed, every Low + Nit + Medium resolved, deferred-with-rationale, or explicitly-noted with current-state documentation"** as the clean finish line. The audit docs now read cleanly top-to-bottom: every C/H is resolved or deferred-with-rationale, every L/N/M is resolved or has a Current-state note explaining why it stays open. The `_lib/*.js` centralizations (supabase.js error contract, github.js path allowlist) give the project two enforceable invariants going forward. That's a defensible resting state.
